...
router.get('/:candidateId', async (req, res, next) => {
  try {
    ...
    if (cacheObj[candidateId]) {
      ...
      const percentile = findPercentile(
        similarCandidates,
        singleCandidate.communication_score,
        singleCandidate.coding_score
      );
      const retObj = {
        title: singleCandidate.title,
        comPercentile: percentile.comPercentile,
    ...
    include: [
      {
        ...
        on: {
          col1: Sequelize.where(
            Sequelize.col('candidate.company_id'),
            '=',
...
const findSimilarCandidiates = async (candidateObj) => {
  ...
  const similarCandidates = candidates
    ...
    .filter((obj) => {
      return Math.abs(candidateObj.company.fractal_index - obj.fractal) < 0.15;
    });
